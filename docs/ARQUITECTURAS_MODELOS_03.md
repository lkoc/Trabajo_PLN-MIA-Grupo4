...
| `03_07` | pantalla completa: 28 individuos + 5 reglas base; insumos y checkpoints originales conservados | CPU x86_64 del runtime Colab | 2 hilos efectivos, 2.000 réplicas por video; 1.397 s antes de escritura | no reentrena |
| `03_07b` | actualización local anidada: 7 variantes; `ensemble_soft_optimized` congelado con pesos 0,10/0,65/0,25; test recombinado sin inferencia | CPU local | 5×5 GroupKFold; grid 0,025 (861 ternas); 2.000 réplicas; cuaderno ejecutado en 370 s | requiere TAR Qwen y checkpoints de test con SHA declarado |
