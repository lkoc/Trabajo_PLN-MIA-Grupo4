...
Un modelo activo debe declarar cinco scores y cinco umbrales principales, el contrato exacto, checkpoint/hash, métrica, split de selección, hardware y linaje. `03_07` acepta únicamente candidatos del SHA-256 activo cuyo test siga sellado; compara individuos y ensembles, informa una frontera de Pareto y congela candidatos, pesos y umbrales. La selección definitiva debe aplicar el criterio normativo de [`CRITERIO_SELECCION_MODELOS_03.md`](CRITERIO_SELECCION_MODELOS_03.md): balanced accuracy binaria a cobertura completa como ranking primario, salvaguardas multietiqueta, incertidumbre pareada por video y `NEEDS_REVIEW` como política posterior con riesgo y cobertura explícitos, nunca como sexta clase. Solo una acción posterior explícita abre test una vez. Publicar exige otra aprobación y no ocurre al comparar. La falta de cualquiera de estas piezas impide registrarlo para producción.
