...
- `01_015` se usa solo para la ampliación minoritaria. Parte de 1.826 chunks de racismo/discriminación y 1.568 de ataque por género/identidad en `train`, pondera déficits de 174 y 432, estima el mínimo de canales PE dirigidos con su rendimiento histórico y añade un margen de canales. En el corte actual calcula 4 canales principales y 239/36/36 candidatos para train/validation/test mediante SHA-256 del `video_id`; el resto del catálogo PE curado queda disponible como reserva. Después de etiquetar, vuelva a ejecutarlo para comprobar si ya se alcanzó la meta de 2.000. Si la disponibilidad inédita no cubre el presupuesto, procesa el lote parcial, conserva el faltante por split y trata de rellenarlo con reservas PE al reanudar; solo se detiene antes de descubrir cuando la capacidad histórica total del catálogo verificado es insuficiente.
