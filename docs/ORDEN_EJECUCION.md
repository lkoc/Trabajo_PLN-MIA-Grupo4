...
|---:|---|---|---|---|
| 1 | `01_01_scraping_incremental` | canales/consultas + candidatos + corpus/caché existente | candidatos y transcripciones JSONL | filtra `video_id`, reutiliza caché y recorre todos los pendientes en lotes reanudables |
...
Agregue candidatos y ejecute otra vez 01→02. Los videos, subtítulos, chunks y etiquetas ya resueltos se omiten. `01_01` adquiere los pendientes por lotes, guarda cada éxito en caché y cada fallo en JSONL antes de avanzar; espera 30 segundos después de cada grupo de 10 respuestas 429 y el tercer grupo sin éxito abre el cortacircuito. Una adquisición correcta reinicia el grupo parcial y los grupos acumulados. `02_05` crea un snapshot nuevo que incluye filas anteriores y nuevas, conserva las asignaciones por `video_id` y no modifica snapshots previos. Los modelos neuronales inicializan el nuevo run desde el candidato compatible anterior y entrenan con el snapshot completo; si no cambió ninguna entrada, todo el tramo permanece en no-op.
