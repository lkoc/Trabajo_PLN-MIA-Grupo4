...
| 5 | `02_01_etiquetado_local_ollama` | chunks pendientes | calibración Flash–Pro, primera pasada Flash y revisión Pro dirigida | firma + `chunk_id`, lotes 5×32, cuarentena y tope de costo |
| 6 | `02_02_etiquetado_remoto` | chunks pendientes | fallback local Qwen3-1.7B | opcional, separado por campaña y `chunk_id` |
| 7 | `02_03_revision_llm_dirigida` | artefactos de `02_01` | tablas de calibración, cobertura y enrutamiento | solo lectura; no repite API |
...
- En `02_01`, active primero `RUN_API_PREFLIGHT=True`, luego `RUN_CALIBRATION=True` sobre 1 000 pares, después `RUN_PRIMARY=True` y al final `RUN_DIRECTED_REVIEW=True`. `PRIMARY_LIMIT=300` y `REVIEW_LIMIT=500` reproducen los pilotos históricos; `None` procesa todos los pendientes. Cada fase es reanudable, muestra tokens/costo y no considera el acuerdo Pro como verdad humana.
