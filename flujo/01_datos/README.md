...
`01_01` reúne el scraping inicial y la antigua ampliación dirigida. Su bloque de controles permite elegir `DISCOVERY_MODE="seed"`, `"directed"` o `"both"`; editar canales y consultas; ampliar la cobertura de búsqueda; y configurar reintentos, lote y pausa. `directed` usa los déficits de `train+validation`, rendimiento histórico por canal, expansión desde búsquedas y una cohorte aislada; sin etiquetas previas activa un fallback equitativo para los cuatro daños. `MAX_DIRECTED_CANDIDATES=None` conserva toda la cohorte inédita y `MAX_NEW_VIDEOS=None` recorre todos sus pendientes en lotes de 50, con 20 segundos entre lotes y 1–3 segundos dentro de `yt-dlp`. `DISCOVER_NEW=False` no consulta fuentes y `FETCH_NEW=False` no obtiene subtítulos.
...
El cuaderno presenta una barra por fuentes durante el descubrimiento y otra por videos durante la adquisición. Esta última distingue intentos, racha, grupos y enfriamientos 429, además de candidatos diferidos. Cada grupo de 10 respuestas 429 produce una espera de 30 segundos; solo tres grupos sin éxito abren el circuito, y cualquier adquisición correcta reinicia ambos contadores. Una URL de canal con HTTP 404 o sin pestaña `/videos` se clasifica como `stale_channel_or_no_videos_tab`; afecta esa fuente, no invalida toda la corrida.
