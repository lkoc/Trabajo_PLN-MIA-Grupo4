...
Esta carpeta contiene el staging local y versionable del bundle reproducible. `project_core.zip`, `chunks_v2.jsonl.gz` y `dataset_5_salidas.jsonl.gz` se regeneran con `tools/prepare_colab_bundle.py`; `bundle_manifest.json` registra tamaños, SHA-256 y un `bundle_id` estable derivado del contenido. `02_00_preparacion_bundle_colab.ipynb` consume este bundle ya construido: lo descarga de GitHub o recibe sus cuatro archivos mediante el navegador.
