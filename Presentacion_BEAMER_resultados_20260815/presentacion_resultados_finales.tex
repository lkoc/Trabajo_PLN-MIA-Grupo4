...
\begin{frame}[shrink=4]{Métricas de ranking: qué miden}
  ...
  \begin{tabularx}{\textwidth}{@{}>{\bfseries\color{Navy}}p{30mm}p{36mm}X>{\centering\arraybackslash}p{15mm}@{}}
    ...
    \rowcolor{Blue!5}
    $R_{0,67}$ & $0,67\,\text{FNR}+0,33\,\text{FPR}$ & Sensibilidad local al costo: penaliza aproximadamente el doble omitir daño que alertar sobre contenido seguro. & $\downarrow$ \\
  ...
  \end{tabularx}
  \vspace{2mm}
  \takeaway{La métrica principal es BA de ANY\_DAMAGE OOF a cobertura completa. Macro-AUPRC evita degradar los cuatro daños; F1 y revisión explican la decisión ya umbralizada.}
...
\begin{frame}[shrink=5]{Cómo leer el ranking y por qué no usar otras métricas}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.48\textwidth}
      \smallcard{Teal}{BA 0,840}{No significa “84\,\% de chunks correctos”: es el promedio de sensibilidad y especificidad de la compuerta binaria, estimado fuera de muestra (OOF).}
      \vspace{1.5mm}
      \smallcard{Gold}{Macro-AUPRC 0,555}{No significa “55,5\,\% de aciertos”: es el área precisión--sensibilidad media de los cuatro daños; solo se compara sobre el mismo panel y prevalencia.}
      \vspace{1.5mm}
      \smallcard{Purple}{Macro-F1 0,568}{Describe las decisiones por categoría en los umbrales congelados; una categoría rara pesa lo mismo que una frecuente.}
    \end{column}
    \begin{column}{.48\textwidth}
...
\begin{frame}[shrink=7]{Criterio de selección y análisis estadístico}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{.38\textwidth}
      \smallcard{Blue}{1 · BA decide}{Promedio simple 0,840; ponderado 0,838; Qwen 0,831. La diferencia simple--Qwen es +0,0086.}
      \vspace{2mm}
      \smallcard{Gold}{2 · Salvaguarda y Pareto}{AUPRC 0,555 frente a 0,516 del mejor individuo: +0,039 y salvaguarda superada. Simple y ponderado quedan en la frontera: uno lidera BA; el otro, AUPRC 0,556.}
      \vspace{2mm}
      \smallcard{Purple}{3 · Desempate de riesgo}{$R_{0,67}=0,67(0,127)+0,33(0,193)=0,149$ para el simple; 0,152 para el ponderado. Menor es preferible.}
    \end{column}
  \end{columns}
  \takeaway{El promedio simple queda primero por BA y menor riesgo; el contraste con el ponderado sigue inconcluso: $\Delta$BA = $-0,0017$, IC95\,\% $[-0,0044;\,0,00083]$, $p_{Holm}=0,23$.}
  \externalNote{Bootstrap pareado por video~\cite{field2007clusterbootstrap}; comparación múltiple~\cite{dror2018significance,holm1979multiple}; criterio interno: \texttt{docs/CRITERIO\_SELECCION\_MODELOS\_03.md}.}
\end{frame}
...
\begin{frame}[shrink=5]{Análisis crítico: ¿el trabajo está en el estado del arte?}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.48\textwidth}
      \smallcard{Green}{Sí es comparable en alcance científico}{Aborda clasificación de lenguaje dañino en contenido social, reporta F1/BA/AUPRC y obtiene F1 0,656 en una vista 4:1: magnitud próxima a DETOXIS y HateXplain.}
      \vspace{2mm}
