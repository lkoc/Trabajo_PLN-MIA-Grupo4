...
\title{Moderación semiautomática de videos peruanos de YouTube mediante modelos clásicos, Transformers, Qwen y ensembles}
\subtitle{Del etiquetado trazable a un sistema humano--IA con desempeño verificable}
...
\hypersetup{
  pdftitle={Moderación semiautomática de videos peruanos de YouTube: resultados finales},
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north west,align=left,text=Navy,font=\fontsize{15}{18}\selectfont\bfseries]
      ...
      {Moderación semiautomática de videos peruanos\\
       de YouTube mediante modelos clásicos,\\
       Transformers, Qwen y ensembles};
    \node[anchor=north west,align=left,text=Ink,font=\small]
      at ([xshift=7mm,yshift=-56mm]current page.north west)
    ...
    \node[anchor=north west,align=left,text=Muted,font=\footnotesize]
      at ([xshift=7mm,yshift=-69mm]current page.north west)
...
\begin{frame}{Formulación del problema en tres niveles}
  \centering
  \begin{tikzpicture}[
    b/.style={draw,rounded corners=4pt,text width=42mm,minimum height=25mm,align=left,inner sep=3mm,font=\small},
    ...
    \node[b,draw=Gold!55,fill=Gold!9,right=6mm of real] (sub) {\pill{Gold}{Nivel subyacente}\par\smallskip
      Datos dispersos, rótulos subjetivos, desbalance, deriva y costo desigual entre omitir daño y revisar de más.};
    \node[b,draw=Blue!50,fill=Blue!7,right=6mm of sub] (model) {\pill{Blue}{Nivel modelado}\par\smallskip
...
\begin{frame}{Objetivos y capacidades evaluadas}
  ...
  \begin{tikzpicture}[node distance=2.5mm,
    ...
    \node[b,draw=Purple!45,fill=Purple!7,below=of o1] (o2) {\textbf{Comparación común} $\rightarrow$ 28 individuos + 5 ensembles bajo el mismo snapshot y criterio de validation.};
...
\begin{frame}{Taxonomía de clasificación}
  ...
  \begin{tikzpicture}[
    ...
    leaf/.style={draw,rounded corners=3pt,text width=25mm,minimum height=20mm,align=center,inner sep=1.5mm,font=\tiny},
    a/.style={-{Stealth[length=1.7mm]},thick,draw=Muted}]
    \node[root] (chunk) {Chunk contextualizado};
    \node[leaf,draw=Teal!55,fill=Teal!7] (safe) at (-60mm,-24mm) {\textbf{SEGURO}\par sin evidencia suficiente};
    \node[leaf,draw=Coral!55,fill=Coral!7] (har) at (-30mm,-24mm) {\textbf{ACOSO / AMENAZA}\par hostigamiento o intimidación};
    \node[leaf,draw=Gold!60,fill=Gold!9] (rac) at (0mm,-24mm) {\textbf{RACISMO / DISCRIMINACIÓN}\par origen o rasgo protegido};
    \node[leaf,draw=Purple!55,fill=Purple!7] (gen) at (30mm,-24mm) {\textbf{GÉNERO / IDENTIDAD}\par violencia o menosprecio};
    \node[leaf,draw=Blue!55,fill=Blue!7] (sex) at (60mm,-24mm) {\textbf{CONTENIDO SEXUAL}\par referencia explícita o inapropiada};
  ...
  \externalNote{Contexto peruano sobre racismo y violencia digital~\cite{callirgos1993racismo,vich2018dinamicas,albornoz2018conocer,defensoria2021violenciaenlinea}; contrato interno: Taxonomía v2.}
\end{frame}
...
\begin{frame}[shrink=5]{Frontend de etiquetado}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.64\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=47mm,keepaspectratio]{assets/captura_entorno_etiquetado.png}
    \end{column}
...
\section{3. Entrenamiento y ensemble}

\begin{frame}{Familias de modelos evaluadas}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.48\textwidth}
  ...
  \begin{center}
    ...
    \pill{Purple}{33 comparados en validation}
  \end{center}
...
\begin{frame}{Diseño del entrenamiento y evaluación}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \end{column}
    \begin{column}{.31\textwidth}
      \smallcard{Gold}{Métricas adecuadas}{Área bajo la curva precisión--recall (AUPRC) para daños raros; BA para la compuerta desbalanceada~\cite{saito2015pr,davis2006pr,brodersen2010balanced}.}
    \end{column}
    \begin{column}{.31\textwidth}
...
\begin{frame}{Adaptación de Qwen mediante LoRA}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \end{column}
    \begin{column}{.31\textwidth}
      \metriccard{Teal}{0,553}{F1 macro}{media armónica de precisión y recall}
    \end{column}
  \end{columns}
  \takeaway{Qwen fue el mejor individuo por familia y también una fuente de diversidad para el ensemble ganador.}
  \mixedNote{Adaptación eficiente~\cite{hu2022lora}; familia Qwen~\cite{qwen2025qwen3}.}{Candidato \texttt{qwen\_lora-4aa5ce04df05}, métricas OOF de validation.}
\end{frame}
...
\begin{frame}[shrink=8]{Metodología del ensemble por promedio de probabilidades}
  ...
  \begin{tikzpicture}[
    ...
    a/.style={-{Stealth[length=2mm]},very thick,draw=Muted}]
    \node[n,draw=Blue!50,fill=Blue!7] (cl) {\textbf{Clásico}\par regresión logística};
...
\begin{frame}{Ranking global en validación}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.69\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=50mm,keepaspectratio]{assets/ranking_global_validation.png}
    \end{column}
...
\begin{frame}{Frontera de Pareto e incertidumbre estadística}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.62\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=47mm,keepaspectratio]{assets/frontera_ba_macro_auprc_validation.png}
    \end{column}
...
\begin{frame}[shrink=5]{Resultados en test de prevalencia natural}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.31\textwidth}
      \metriccard{Teal}{0,846}{balanced accuracy}{cobertura completa}
    \end{column}
    \begin{column}{.31\textwidth}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{.46\textwidth}
      \smallcard{Gold}{Costo del recall}{La especificidad es 0,786: la política prioriza detectar daño y envía más falsos positivos a revisión.}
      \vspace{2mm}
...
\begin{frame}[shrink=5]{Política de revisión humana}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.55\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ybar,width=\linewidth,height=52mm,ymin=0,ymax=1,
...
\begin{frame}[shrink=5]{Mejor modelo por categoría en validación}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.56\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=49mm,keepaspectratio]{assets/seleccion_por_categoria_validation.png}
    \end{column}
    \begin{column}{.41\textwidth}
      \scriptsize
  ...
  \resultNote{Validation, \texttt{ganadores\_por\_categoria\_validation.csv}; estas selecciones descriptivas no reabren el test.}
\end{frame}
...
\begin{frame}[shrink=5]{Comparación contextual con el estado del arte}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.62\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          xbar,width=\linewidth,height=54mm,xmin=.55,xmax=.84,
...
\begin{frame}[shrink=5]{Frontend de producción}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.64\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=51mm,keepaspectratio]{assets/captura_entorno_operacion.png}
    \end{column}
    \begin{column}{.33\textwidth}
...
\begin{frame}{Conclusiones vinculadas a los objetivos I}
  ...
  \begin{tabularx}{\textwidth}{@{}p{25mm}X X@{}}
    ...
    Se logró una cadena humano--IA funcional: prioriza daño con alto recall y conserva revisión para incertidumbre.\\[2mm]
...
\begin{frame}[shrink=5]{Limitaciones y trabajo futuro}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.48\textwidth}
  ...
  \centering
  \begin{tikzpicture}[node distance=3mm,
    n/.style={draw=Teal!45,fill=Teal!7,rounded corners=3pt,text width=29mm,minimum height=13mm,align=center,font=\scriptsize},
    a/.style={-{Stealth[length=1.7mm]},thick,draw=Muted}]
