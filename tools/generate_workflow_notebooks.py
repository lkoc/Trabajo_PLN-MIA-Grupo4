...
def main(*, only_notebooks: set[str] | None = None) -> None:
    ...
    training_notebooks = [
        (
            ...
            "Modelos clásicos",
            "from moderacion_peru.experiments import train_classical_experiments\nDATA=ROOT/'datos/model_ready/v2/dataset_5_salidas.jsonl'\nOUTPUT=ROOT/'modelos/v2/clasicos'\nSAFE_TO_DAMAGE_RATIO=4.0  # política fija en train y validation\nPARALLEL_WORKERS=4  # 4/16 hilos: comparte la matriz dispersa sin saturar RAM\nRUN_TRAINING=False\nRUN_CHANNEL_ROBUSTNESS=False\nif RUN_TRAINING:\n    show_result('Clásicos base e informados por política',train_classical_experiments(DATA,OUTPUT,variants=('base','policy_informed'),safe_to_damage_ratio=SAFE_TO_DAMAGE_RATIO,parallel_workers=PARALLEL_WORKERS),tone='success')\nif RUN_CHANNEL_ROBUSTNESS:\n    show_result('Robustez con canales retenidos',train_classical_experiments(DATA,OUTPUT/'channel_heldout',model_names=('logistic_regression',),variants=('base','policy_informed'),safe_to_damage_ratio=SAFE_TO_DAMAGE_RATIO,split_scheme='channel',parallel_workers=PARALLEL_WORKERS),tone='success')\nif not (RUN_TRAINING or RUN_CHANNEL_ROBUSTNESS):\n    show_summary('Entrenamiento desactivado',{'salidas':'22 enmascaradas','SEGURO_train_validation':'4:1','TF-IDF':'una extracción por variante, reutilizada por cinco modelos','paralelismo':f'{PARALLEL_WORKERS} hilos compartiendo matriz dispersa','test':'natural completo, sellado'},tone='neutral')",
            None,
        ...
        (
            ...
            "Transformers planos",
            "from moderacion_peru.experiments import train_flat_transformers,train_neural_experiment\nDATA=COLAB_CONTEXT.input('dataset_5_salidas') if COLAB_CONTEXT else ROOT/'datos/model_ready/v2/dataset_5_salidas.jsonl'\nOUTPUT_ROOT=COLAB_CONTEXT.scratch_output_dir if COLAB_CONTEXT else ROOT/'modelos/v2/transformers_planos'\nDEVICE='cuda' if COLAB_CONTEXT else 'auto'\nSAFE_TO_DAMAGE_RATIO=4.0\nRUN_TRAINING=False\nRUN_CHANNEL_ROBUSTNESS=False\nif RUN_TRAINING:\n    show_result('Transformers planos 22 salidas',train_flat_transformers(DATA,OUTPUT_ROOT,device=DEVICE,safe_to_damage_ratio=SAFE_TO_DAMAGE_RATIO),tone='success')\nif RUN_CHANNEL_ROBUSTNESS:\n    show_result('MiniLM con canales retenidos',train_neural_experiment(DATA,OUTPUT_ROOT/'channel_heldout',experiment='flat_minilm',device=DEVICE,safe_to_damage_ratio=SAFE_TO_DAMAGE_RATIO,split_scheme='channel'),tone='success')\nif not (RUN_TRAINING or RUN_CHANNEL_ROBUSTNESS):\n    show_summary('Configuración preliminar',{'datos':DATA,'salida':OUTPUT_ROOT,'SEGURO_train_validation':'4:1 fijo','salidas':'5+14+3 enmascaradas','test':'natural completo, sellado','acción':'Active RUN_TRAINING; early stopping usa solo validation.'},tone='neutral')",
            "03_02",
        ...
        (
            ...
            "from moderacion_peru.experiments import train_neural_experiment\nDATA=COLAB_CONTEXT.input('dataset_5_salidas') if COLAB_CONTEXT else ROOT/'datos/model_ready/v2/dataset_5_salidas.jsonl'\nOUTPUT_ROOT=COLAB_CONTEXT.scratch_output_dir if COLAB_CONTEXT else ROOT/'modelos/v2/cascada'\nDEVICE='cuda' if COLAB_CONTEXT else 'auto'\nSAFE_TO_DAMAGE_RATIO=4.0\nRUN_TRAINING=False\nif RUN_TRAINING:\n    show_result('Cascada con auxiliares',train_neural_experiment(DATA,OUTPUT_ROOT,experiment='cascade',device=DEVICE,safe_to_damage_ratio=SAFE_TO_DAMAGE_RATIO),tone='success')\nelse:\n    show_summary('Configuración preliminar',{'compuerta':'cualquier daño + 14 finas + 3 flags','rama_daño':4,'diagnóstico':'propagación de error; comparación plana en 03_07','SEGURO_train_validation':'4:1 fijo','test':'natural completo, sellado'},tone='neutral')",
            "03_03",
        ...
        (
            ...
            "from moderacion_peru.experiments import train_neural_experiment\nDATA=COLAB_CONTEXT.input('dataset_5_salidas') if COLAB_CONTEXT else ROOT/'datos/model_ready/v2/dataset_5_salidas.jsonl'\nOUTPUT_ROOT=COLAB_CONTEXT.scratch_output_dir if COLAB_CONTEXT else ROOT/'modelos/v2/multitarea'\nDEVICE='cuda' if COLAB_CONTEXT else 'auto'\nSAFE_TO_DAMAGE_RATIO=4.0  # política fija en train y validation\nRUN_TRAINING=False\nif RUN_TRAINING:\n    show_result('Multitarea enmascarada',train_neural_experiment(DATA,OUTPUT_ROOT,experiment='multitask',device=DEVICE,safe_to_damage_ratio=SAFE_TO_DAMAGE_RATIO),tone='success')\nelse:\n    show_summary('Configuración preliminar',{'salidas':'5 gruesas + 14 finas + 3 flags','máscaras':'ausente no equivale a negativo','SEGURO_train_validation':'4:1 fijo','test':'natural completo, sellado'},tone='neutral')",
            "03_04",
        ...
        (
            ...
            "Qwen-LoRA",
            "from moderacion_peru.experiments import train_neural_experiment\nDATA=COLAB_CONTEXT.input('dataset_5_salidas') if COLAB_CONTEXT else ROOT/'datos/model_ready/v2/dataset_5_salidas.jsonl'\nOUTPUT_ROOT=COLAB_CONTEXT.scratch_output_dir if COLAB_CONTEXT else ROOT/'modelos/v2/qwen_lora'\nDEVICE='cuda' if COLAB_CONTEXT else 'auto'\nRUN_TRAINING=False\nif RUN_TRAINING:\n    show_result('Qwen-LoRA clasificador de 22 salidas',train_neural_experiment(DATA,OUTPUT_ROOT,experiment='qwen_lora',device=DEVICE,safe_to_damage_ratio=4.0),tone='success')\nelse:\n    show_summary('Configuración preliminar',{'método':'clasificador supervisado, no condicionado por prompt','salidas':'5+14+3 enmascaradas','SEGURO_train_validation':'4:1 fijo','reanuda_colab':bool(COLAB_CONTEXT and COLAB_CONTEXT.resumed),'test':'natural completo, sellado'},tone='neutral')",
            "03_05",
        ...
        (
            ...
            "Qwen estructurado",
            "from moderacion_peru.experiments import train_neural_experiment\nDATA=COLAB_CONTEXT.input('dataset_5_salidas') if COLAB_CONTEXT else ROOT/'datos/model_ready/v2/dataset_5_salidas.jsonl'\nOUTPUT_ROOT=COLAB_CONTEXT.scratch_output_dir if COLAB_CONTEXT else ROOT/'modelos/v2/qwen_estructurado'\nDEVICE='cuda' if COLAB_CONTEXT else 'auto'\nRUN_TRAINING=False\nif RUN_TRAINING:\n    show_result('Qwen estructurado 22 salidas',train_neural_experiment(DATA,OUTPUT_ROOT,experiment='qwen_structured',device=DEVICE,safe_to_damage_ratio=4.0),tone='success')\nelse:\n    show_summary('Configuración preliminar',{'estructura':'penaliza conflicto SEGURO+daño','auxiliares':'enmascaradas','SEGURO_train_validation':'4:1 fijo','calibración':'validation','test':'natural completo, sellado'},tone='neutral')",
            "03_06",
