...
def apply_citations(cells: Iterable[Any], metadata: dict[str, Any]) -> Any:
    ...
    if not ordered_keys:
        raise ValueError("Cada cuaderno activo debe contener al menos una cita académica")
    numbers = {key: index for index, key in enumerate(ordered_keys, start=1)}
    for cell in cells:
        if cell.cell_type == "markdown":
            cell.source = CITATION_MARKER.sub(lambda match: f"[{numbers[match.group(1)]}]", cell.source)
