...
| `01_02_optimizacion_longitud_chunks` | chunks etiquetados, transcripciones y snapshot entrenable | piloto rápido o confirmación corta para 15/20/25/30/35 s; comparación neuronal acotada con MiniLM congelado y Gemma 3 4B | opcional; la comparación neuronal es informativa, nunca selecciona con `test` ni cambia datos si `APPLY_CHUNK_SELECTION=False` |
...
| `01_02` | pruebas CPU, comparadores neuronales y aplicación están en `False`; elección manual 30 s | active primero el smoke rápido; después puede activar MiniLM/Gemma de forma acotada o la confirmación corta; solo `APPLY_CHUNK_SELECTION=True` archiva/restaura derivados y escribe la elección |
