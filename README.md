...
| `01_02_optimizacion_longitud_chunks` | chunks etiquetados, transcripciones y snapshot entrenable | compara 15/20/25/30/35 s mediante smoke, confirmación o perfil robusto; ofrece diagnósticos acotados con MiniLM congelado y Gemma 3 4B | opcional; el perfil robusto usa cinco cohortes y bootstrap por video, nunca selecciona con `test` ni cambia datos si `APPLY_CHUNK_SELECTION=False` |
...
`01_02` está separado de la materialización porque elegir longitud es selección de hiperparámetros. El smoke test rápido entrena dos baselines CPU una vez por longitud; el modo confirmatorio entrena ComplementNB, regresión logística y SGD en tres cohortes pareadas. El perfil robusto usa 300/100/100 videos, cinco cohortes, 75 ajustes y 1 000 réplicas bootstrap agrupadas por `video_id`. Compara cada longitud contra la referencia predeclarada de 30 s con un margen de no inferioridad de 0.01 AP y reserva `test` para descripción. En todos los perfiles, entrenamiento, calibración e inferencia usan chunks de la misma longitud. Como diagnóstico opcional, `RUN_BOUNDED_HF_COMPARISON` compara 20 s y 30 s mediante embeddings MiniLM congelados y una cabeza logística; `RUN_BOUNDED_OLLAMA_COMPARISON` consulta `gemma3:4b` sobre solo tres filas por longitud, con reanudación y un presupuesto total de diez minutos. Esta última informa latencia, validez JSON y etiquetas duras, pero no modifica la recomendación porque no es directamente comparable con la AP continua de MiniLM. La confirmación ampliada del 6 de agosto de 2026 eligió conservar **30 s** con AP `0.1142 ± 0.0050`. El perfil robusto del 7 de agosto confirmó la decisión: 30 s obtuvo AP `0.1233`, IC bootstrap 95% `[0.1099, 0.1446]`, y fue la única longitud no inferior; completó en aproximadamente 14 minutos. Todas son métricas proxy, no productivas. La metodología, los resultados por modelo y la trazabilidad están en [Informe de selección de longitud de chunks](docs/OPTIMIZACION_LONGITUD_CHUNKS.md).

Durante `01_01`, primero se consolidan en el canónico todas las transcripciones completas disponibles en snapshots, particiones por canal y cachés. También se consolidan los VTT locales, se regenera su índice y se calcula la diferencia exacta por `video_id`: el backfill de VTT conocidos se procesa antes que los candidatos nuevos. Después se inventarían datasets y chunks históricos con texto; estos derivados no se presentan como raw, pero sus `video_id` se excluyen para impedir descargas duplicadas. La corrida actual continúa incrementando las particiones por canal y los VTT. El último índice VTT anterior a esas descargas registraba 3 274 archivos para 3 270 videos y 983 faltantes; esa cifra es un checkpoint y debe recalcularse al terminar, no un resultado consolidado. Una barra muestra la fuente actual y avanza al terminar cada canal o consulta; otras informan el backfill y los candidatos. El descubrimiento aplica un timeout HTTP configurable de 30 segundos por operación y guarda atómicamente cada fuente terminada en `datos/raw/manifests/discovery_<modo>_checkpoint.json`. Al reanudar, reutiliza las fuentes exitosas sin red y vuelve a intentar únicamente las fallidas. El modo `directed` calcula déficits por videos etiquetados de `train+validation`, estima rendimiento histórico por canal, expande canales desde búsquedas temáticas y materializa una cohorte vigente con *round-robin* ponderado; si no hay datos previos, reparte la adquisición por igual entre los cuatro daños. Con `MAX_DIRECTED_CANDIDATES=None`, `MAX_VTT_BACKFILL=None` y `MAX_NEW_VIDEOS=None` se recorren las colas completas. La cola de descarga usa una prioridad pseudoaleatoria reproducible e intercala un video por canal. La red se regula en lotes de 10, con una pausa adicional de 15 segundos entre lotes y pausas internas de 2.5–10 segundos en `yt-dlp`. El detalle queda en `datos/raw/fallos_descubrimiento_ultima_ejecucion.json`, `datos/raw/fallos_vtt_backfill.jsonl` y `datos/raw/fallos_adquisicion.jsonl`.
...
Los 17 cuadernos vigentes se encuentran exclusivamente bajo `flujo/`. Las
...
| `01_02` | smoke, confirmación, perfil robusto, comparadores neuronales y aplicación están en `False`; elección manual 30 s | para la prueba mayor active solo `RUN_CHUNK_LENGTH_ROBUST_TEST=True`: ejecuta 75 ajustes y 1 000 réplicas por video; audite los JSON antes de activar `USE_ROBUST_RECOMMENDATION` y mantenga `APPLY_CHUNK_SELECTION=False` hasta aprobar el cambio |
