...
> **Enmienda final v2.1 (2026-08-15).** El entrenamiento y la evaluación de las cinco salidas ya concluyeron. Los artefactos canónicos de `03_07` y `03_07a` sustituyen, para el manuscrito vigente, la indicación histórica de métricas pendientes de la enmienda del 5 de agosto. La selección congelada es `ensemble_soft_mean`; su estado inferencial es `statistical_tie_or_inconclusive`, y el test permanece abierto una sola vez conforme al protocolo. Las cifras publicables deben trazarse a `resultados/modelos/seleccion_congelada.json`, `comparacion_individual_ensemble_validation.json`, `test_final_abierto_una_vez.json` y `resumen_03_07a.json`.
