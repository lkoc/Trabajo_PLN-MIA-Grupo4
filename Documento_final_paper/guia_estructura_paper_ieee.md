...
> **Enmienda final v2.1 (actualizada 2026-08-17).** El entrenamiento y la evaluación de las cinco salidas ya concluyeron. `03_07b` amplía la comparación única con validación anidada de cinco reglas base y dos mezclas optimizadas. La selección congelada vigente es `ensemble_soft_optimized`, con pesos clásico/Transformer/Qwen 0,10/0,65/0,25; su ventaja pareada frente al retador es inconclusa, y el test conserva una sola apertura. Las cifras publicables deben trazarse a `resultados/modelos/seleccion_congelada.json`, `comparacion_individual_ensemble_validation.json`, `optimizacion_ensembles/optimizacion_ensembles_validation.json`, `test_final_abierto_una_vez.json` y `resumen_03_07a.json`.
...
En este entregable, por decisión editorial, las referencias se ubican inmediatamente después del cuerpo principal y los apéndices comienzan a continuación, cada uno en una página nueva y con un título que identifique su contenido. Cite cada apéndice desde el cuerpo, en el lugar donde amplía datos, método, resultados, trazabilidad o disponibilidad; una tabla o figura etiquetada dentro del apéndice no reemplaza esa referencia entrante. Puede cambiar a una columna para prompts, ejemplos o tablas que lo necesiten. Tras compilar, compruebe saltos, páginas vacías, numeración y referencias cruzadas; si la publicación de destino exige otra secuencia, prevalece su plantilla.
